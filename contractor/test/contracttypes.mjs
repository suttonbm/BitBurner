...
export const codingContractTypesMetadata = [
  ...
  },
  {
    ...
    gen: () => {
      const intervals = [];
      const numIntervals = getRandomInt(3, 20);
      for (let i = 0; i < numIntervals; ++i) {
        const start = getRandomInt(1, 25);
        const end = start + getRandomInt(1, 10);
  ...
  },
  {
    ...
    gen: () => {
      const triangle = [];
      const levels = getRandomInt(3, 12);
  ...
  },
  {
    ...
    difficulty: 3,
    gen: () => {
      const numRows = getRandomInt(2, 14);
      const numColumns = getRandomInt(2, 14);
  ...
  },
  {
    ...
    gen: () => {
      ...
      const target = getRandomInt(-100, 100);
